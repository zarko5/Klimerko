...
//#include "src/Images/Weather-icon.bmp"
...
int displayMode = 0; // 0=PM2.5, 1=PM10, 2=Env, 3=PM1
...
void drawCircularGauge(int value, int minVal, int maxVal, int x, int y, int r) {
  // Draw background arc (dimmed if possible, but here just outline or dotted)
  // Since we only have 1 color, we'll use a thin arc for background
  // and a thick arc for the value.
  
  int angleStart = 135;
  int angleEnd = 405;
  int angleRange = angleEnd - angleStart;
  
  // Map value to angle
  if (value < minVal) value = minVal;
  if (value > maxVal) value = maxVal;
  
  int valAngle = map(value, minVal, maxVal, angleStart, angleStart + angleRange);
  
  // Draw background arc (simulated with dots or thin line)
  // For simplicity on GFX, we can just draw the full range as a thin line
  // display.drawCircle(x, y, r, SH110X_WHITE); // Too simple
  
  // Draw the "filled" part (thick arc)
  // GFX doesn't have thick arc, so we draw multiple triangles or lines
  // Or simpler: draw multiple arcs with decreasing radius
  
  for (int i = angleStart; i <= valAngle; i+=2) {
      float rad = i * PI / 180;
      int x1 = x + cos(rad) * (r - 4);
      int y1 = y + sin(rad) * (r - 4);
      int x2 = x + cos(rad) * r;
      int y2 = y + sin(rad) * r;
  ...
  // Draw the "empty" part
   for (int i = valAngle; i <= angleEnd; i+=6) {
      float rad = i * PI / 180;
      int x1 = x + cos(rad) * (r - 1);
      int y1 = y + sin(rad) * (r - 1);
      display.drawPixel(x1, y1, SH110X_WHITE);
  }
}

void drawCenteredText(String text, int x, int y, const GFXfont *font) {
    int16_t x1, y1;
    uint16_t w, h;
...
void drawWeatherIcon(int x, int y, float pressure) {
    // Simple weather icons based on pressure
    // > 1020: Sunny
    // < 1000: Rainy
    // Else: Cloudy
    ...
    if (pressure > 1020) {
        ...
        for (int i=0; i<8; i++) {
            ...
            int x1 = x + cos(angle) * 8;
            int y1 = y + sin(angle) * 8;
        ...
        // Rainy
        // Cloud
        display.fillCircle(x-4, y-2, 4, SH110X_WHITE);
        display.fillCircle(x+4, y-2, 4, SH110X_WHITE);
        display.fillCircle(x, y-5, 5, SH110X_WHITE);
        // Drops
        display.drawLine(x-4, y+4, x-6, y+8, SH110X_WHITE);
        display.drawLine(x, y+4, x-2, y+8, SH110X_WHITE);
        display.drawLine(x+4, y+4, x+2, y+8, SH110X_WHITE);
    } else {
        // Cloudy
        display.drawCircle(x-4, y, 4, SH110X_WHITE);
        display.drawCircle(x+4, y, 4, SH110X_WHITE);
        display.drawCircle(x, y-3, 5, SH110X_WHITE);
        // Clear inside to make it look like an outline cloud
        display.fillCircle(x-4, y, 3, SH110X_BLACK);
        display.fillCircle(x+4, y, 3, SH110X_BLACK);
        display.fillCircle(x, y-3, 4, SH110X_BLACK);
    }
}
...
void updateDisplay() {
    ...
    if (!wifiConnectionLost) {
        ...
        int bars = 0;
        if (signal > -55) bars = 4;
        else if (signal > -70) bars = 3;
        else if (signal > -80) bars = 2;
        else if (signal > -90) bars = 1;
        
        for (int i=0; i<4; i++) {
            if (i < bars) display.fillRect(110 + (i*4), 10 - (i*2), 3, (i*2)+2, SH110X_WHITE);
            else display.drawRect(110 + (i*4), 10 - (i*2), 3, (i*2)+2, SH110X_WHITE);
        }
    } else {
        drawWeatherIcon(118, 10, avgPressure);
    ...
    // Header Text
    display.setFont(NULL); // Default font for small text
    display.setCursor(0, 0);
    display.print("KLIMERKO");

    // ---------------- CENTERPIECE  ----------------
    // Draw Gauge
    drawCircularGauge(avgPM25, 0, 100, 64, 64, 40);
    
    // Draw Value

    display.setTextColor(SH110X_WHITE);
    String pm25Str = String(avgPM25);
    drawCenteredText(pm25Str, 64, 64, &FreeSansBold24pt7b);
    
    // Label
    drawCenteredText("PM 2.5", 64, 90, &FreeSans9pt7b);

    // ---------------- BOTTOM BAR ----------------
    display.drawLine(0, 100, 128, 100, SH110X_WHITE);
    
    // Temp
    display.setFont(NULL);
    display.setCursor(5, 110);
    display.print("T: "); display.print(avgTemperature, 1); display.print("C");
    
    // Hum
    display.setCursor(70, 110);
    display.print("H: "); display.print(avgHumidity, 0); display.print("%");
